...
/* ---------- الكمية المتبقية لكل منتج (10 إلى 30 قطعة) ----------
   لو المنتج معاه "stock" ثابت في products.js بيتاخد هو، ولو مش موجود
   بيتحسب رقم ثابت تلقائي من الـ id عشان يفضل نفس الرقم طول الوقت. */
function productStock(product){
  if(typeof product.stock === 'number' && product.stock >= 0) return product.stock;
  const seed = (product.id * 15485863 + 2038074743) % 1000;
  return 10 + (seed % 21); // 10..30
}

/* ---------- عداد العرض (من يومين إلى 6 أيام) — بيتخزن في الجهاز عشان
   يفضل بيعد بشكل حقيقي، ولما ينتهي بيبدأ عرض جديد تلقائيًا ---------- */
function offerEndTime(id){
  const key = 'mallwi_offer_' + id;
  let end = parseInt(localStorage.getItem(key), 10);
  const now = Date.now();
  if(!end || isNaN(end) || end <= now){
    const seed = (id * 7919 + 104729) % 5; // 0..4
    const days = 2 + seed; // 2..6 يوم
...
function formatCountdown(ms){
  if(ms <= 0) return { d:0, h:0, m:0, s:0, text:'انتهى العرض' };
...
/* بيحدّث كل عدادات العروض الظاهرة في الصفحة كل ثانية (كروت + المعاينة السريعة) */
function tickCountdowns(){
  document.querySelectorAll('[data-offer-end]').forEach(el=>{
    const end = parseInt(el.getAttribute('data-offer-end'), 10);
    const left = end - Date.now();
    const t = formatCountdown(left);
    const compact = el.classList.contains('cd-compact');
    if(left <= 0){
      el.innerHTML = compact
        ? `<i class="fa-regular fa-clock"></i> العرض هيتجدد قريبًا`
        : `<i class="fa-regular fa-clock"></i> العرض هيتجدد قريبًا`;
      return;
    }
    if(compact){
      el.innerHTML = `<i class="fa-regular fa-clock"></i> ${t.d>0?t.d+'ي ':''}${String(t.h).padStart(2,'0')}:${String(t.m).padStart(2,'0')}:${String(t.s).padStart(2,'0')}`;
    } else {
      el.innerHTML = `
        <div class="cd-box"><span>${String(t.d).padStart(2,'0')}</span><small>يوم</small></div>
        <div class="cd-box"><span>${String(t.h).padStart(2,'0')}</span><small>ساعة</small></div>
        <div class="cd-box"><span>${String(t.m).padStart(2,'0')}</span><small>دقيقة</small></div>
        <div class="cd-box"><span>${String(t.s).padStart(2,'0')}</span><small>ثانية</small></div>
      `;
    }
  });
}
setInterval(tickCountdowns, 1000);
document.addEventListener('DOMContentLoaded', tickCountdowns);
...
/* رابط المشاركة بيوديك لصفحة معاينة ثابتة خاصة بالمنتج (p/ID.html) فيها
   صورة المنتج نفسه في وسوم og:image، عشان لما تتشارك على فيسبوك/واتساب/
   تليجرام تظهر صورة المنتج تلقائيًا في المعاينة. الصفحة دي بتحوّل الزائر
   فورًا لصفحة المنتج الحقيقية (رجالي/حريمي) وتفتحله المعاينة السريعة.
   ملحوظة: صفحات p/ بتتولّد من products.js عن طريق سكربت generate-share-pages.js
   المرفق — شغّله مرة كل ما تضيف/تعدّل منتج قبل ما ترفع الموقع. */
function productShareUrl(product){
  return `${SITE_URL}/p/${product.id}.html`;
}
...
function buildShareCaption(product){
  ...
  const t = formatCountdown(offerEndTime(product.id) - Date.now());
  const sizesLine = (product.sizes && product.sizes.length) ? `📏 المقاسات المتاحة: ${product.sizes.join(' - ')}\n` : '';
  const colorsLine = (product.colors && product.colors.length) ? `🎨 الألوان المتاحة: ${product.colors.join(' - ')}\n` : '';
  const stockLine = `📦 متبقي ${stock} قطعة بس من الكمية دي\n`;
  const timeLine = t.d > 0
    ? `⏳ العرض هينتهي خلال ${t.d} يوم و${t.h} ساعة\n`
    : `⏳ العرض هينتهي خلال ${t.h} ساعة و${t.m} دقيقة\n`;
  return (
`✨ 𝗠𝗔𝗟𝗟𝗪𝗜 𝗕𝗥𝗔𝗡𝗗 ✨
${hook}
...
${sizesLine}${colorsLine}${stockLine}${timeLine}
🚚 شحن لكل محافظات مصر
💵 الدفع عند الاستلام
...
function openShareSheet(product){
  ...
  document.getElementById('share-preview').innerHTML = `
    ...
    <div class="share-preview-info">
      ...
      <div class="share-preview-stock"><i class="fa-solid fa-box"></i> متبقي ${productStock(product)} قطعة فقط</div>
    </div>`;
